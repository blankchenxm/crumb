...
#define SERVER_URL              "http://192.168.117.25:8000/upload"
...
static esp_err_t upload_latest_recording(recording_metadata_t *meta)
{
    ...
    for (uint32_t page = 0; page < meta->pages_used && remaining > 0; page++) {
        size_t chunk = remaining > W25N_PAGE_SIZE ? W25N_PAGE_SIZE : remaining;
        ret = w25n_read_page(page, 0, s_upload_page, chunk);
        if (ret != ESP_OK) {
        ...
        written = esp_http_client_write(client, (const char *)s_upload_page, chunk);
        if (written != (int)chunk) {
            ESP_LOGE(TAG, "HTTP write failed at page %" PRIu32, page);
            esp_http_client_close(client);
            esp_http_client_cleanup(client);
            return ESP_FAIL;
        }
