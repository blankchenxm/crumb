/*
 * Offline recorder for ESP32 + 2x ICS-41350 + W25N01GV.
 *
 * Behavior:
 *   - Press and hold KEY_PIN low to record from both PDM microphones.
 *   - Release the button to stop.
 *   - Raw PCM is stored into W25N01 sequentially.
 *   - Final metadata is written to the last block so the latest recording
 *     can be recovered later.
 *
 * Notes:
 *   - The recording is stored as raw PCM + metadata, not as a filesystem file.
 *   - main2.c extends the same storage format with WiFi upload.
 */
...
static uint8_t s_capture_buf[PDM_READ_BYTES];
...
static esp_err_t w25n_program_page(uint32_t page, const uint8_t *data, size_t len)
{
    ...
    tx[0] = W25N_CMD_PROGRAM_LOAD;
    tx[1] = 0x00;
    tx[2] = 0x00;
    memcpy(tx + 3, data, len);

    ret = w25n_tx(tx, tx_len);
    free(tx);
    if (ret != ESP_OK) {
        return ret;
    }
...
static esp_err_t record_one_session(recording_metadata_t *saved_meta)
{
    ...
    while (key_is_pressed()) {
        ...
        esp_err_t ret = i2s_channel_read(s_rx_chan,
                                         s_capture_buf,
                                         sizeof(s_capture_buf),
                                         &bytes_read,
                                         pdMS_TO_TICKS(I2S_READ_TIMEOUT_MS));
        if (ret == ESP_OK && bytes_read > 0) {
            ret = recording_append(session, s_capture_buf, bytes_read);
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to store audio into W25N: %s", esp_err_to_name(ret));
                return ret;
            }

            session->meta.pcm_bytes += bytes_read;
        } else if (ret != ESP_OK && ret != ESP_ERR_TIMEOUT) {
            ESP_LOGW(TAG, "i2s_channel_read failed: %s", esp_err_to_name(ret));
        }

        TickType_t now = xTaskGetTickCount();
        if ((now - last_progress_tick) >= pdMS_TO_TICKS(PROGRESS_LOG_MS)) {
            double seconds = (double)session->meta.pcm_bytes /
                             (PDM_SAMPLE_RATE * PDM_CHANNELS * (PDM_BITS_PER_SAMPLE / 8));
            ESP_LOGI(TAG, "Recording... %.2f s, %" PRIu32 " bytes", seconds, session->meta.pcm_bytes);
    ...
    esp_err_t ret = recording_flush_page(session);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to flush final page: %s", esp_err_to_name(ret));
        return ret;
    }
...
void app_main(void)
{
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "==============================");
    ESP_LOGI(TAG, "  ESP32 W25N Audio Recorder");
    ESP_LOGI(TAG, "==============================");

    key_init();
    ESP_ERROR_CHECK(pdm_mic_init());
    ESP_ERROR_CHECK(w25n_init());
    xTaskCreate(recorder_task, "recorder_task", RECORDER_TASK_STACK, NULL, 5, NULL);
}
